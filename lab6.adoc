...
== Замена системного таймера на TIM2

=== Что такое системный таймер и зачем его менять
...
=== Как это все работает на пальцах
...
=== Использование внутреннего генератора HSI
...
=== Включение и настройка внешнего кварца (если бы использовали его)
...
=== Настройка PLL (если делать через него)
...
----
Переключает систему на работу от PLL (32 МГц) и ждёт подтверждения. Максимальная частота PLL 100 МГц
